<div class="enderecos index">
	<h2><?php echo __('Enderecos'); ?></h2>
	<table cellpadding="0" cellspacing="0">
	<tr>
			<th><?php echo $this->Paginator->sort('id'); ?></th>
			<th><?php echo $this->Paginator->sort('parceirodenegocio_id'); ?></th>
			<th><?php echo $this->Paginator->sort('tipo'); ?></th>
			<th><?php echo $this->Paginator->sort('logradouro'); ?></th>
			<th><?php echo $this->Paginator->sort('complemento'); ?></th>
			<th><?php echo $this->Paginator->sort('bairro'); ?></th>
			<th><?php echo $this->Paginator->sort('cidade'); ?></th>
			<th><?php echo $this->Paginator->sort('uf'); ?></th>
			<th><?php echo $this->Paginator->sort('longitude'); ?></th>
			<th><?php echo $this->Paginator->sort('altitude'); ?></th>
			<th><?php echo $this->Paginator->sort('ponto_referencia'); ?></th>
			<th class="actions"><?php echo __('Actions'); ?></th>
	</tr>
	<?php foreach ($enderecos as $endereco): ?>
	<tr>
		<td><?php echo h($endereco['Endereco']['id']); ?>&nbsp;</td>
		<td>
			<?php echo $this->Html->link($endereco['Parceirodenegocio']['id'], array('controller' => 'parceirodenegocios', 'action' => 'view', $endereco['Parceirodenegocio']['id'])); ?>
		</td>
		<td><?php echo h($endereco['Endereco']['tipo']); ?>&nbsp;</td>
		<td><?php echo h($endereco['Endereco']['logradouro']); ?>&nbsp;</td>
		<td><?php echo h($endereco['Endereco']['complemento']); ?>&nbsp;</td>
		<td><?php echo h($endereco['Endereco']['bairro']); ?>&nbsp;</td>
		<td><?php echo h($endereco['Endereco']['cidade']); ?>&nbsp;</td>
		<td><?php echo h($endereco['Endereco']['uf']); ?>&nbsp;</td>
		<td><?php echo h($endereco['Endereco']['longitude']); ?>&nbsp;</td>
		<td><?php echo h($endereco['Endereco']['altitude']); ?>&nbsp;</td>
		<td><?php echo h($endereco['Endereco']['ponto_referencia']); ?>&nbsp;</td>
		<td class="actions">
			<?php echo $this->Html->link(__('View'), array('action' => 'view', $endereco['Endereco']['id'])); ?>
			<?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $endereco['Endereco']['id'])); ?>
			<?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $endereco['Endereco']['id']), null, __('Are you sure you want to delete # %s?', $endereco['Endereco']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
	</table>
	<p>
	<?php
	echo $this->Paginator->counter(array(
	'format' => __('Page {:page} of {:pages}, showing {:current} records out of {:count} total, starting on record {:start}, ending on {:end}')
	));
	?>	</p>
	<div class="paging">
	<?php
		echo $this->Paginator->prev('< ' . __('previous'), array(), null, array('class' => 'prev disabled'));
		echo $this->Paginator->numbers(array('separator' => ''));
		echo $this->Paginator->next(__('next') . ' >', array(), null, array('class' => 'next disabled'));
	?>
	</div>
</div>
<div class="actions">
	<h3><?php echo __('Actions'); ?></h3>
	<ul>
		<li><?php echo $this->Html->link(__('New Endereco'), array('action' => 'add')); ?></li>
		<li><?php echo $this->Html->link(__('List Parceirodenegocios'), array('controller' => 'parceirodenegocios', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Parceirodenegocio'), array('controller' => 'parceirodenegocios', 'action' => 'add')); ?> </li>
	</ul>
</div>
