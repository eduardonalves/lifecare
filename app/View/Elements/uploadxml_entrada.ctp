...
<header >
	<?php 
		echo $this->Html->image('titulo-entrada.png', array('class' => 'entrada-icon', 'alt' => 'Entrada ', 'title' => 'Entrada', 'border' => '0'));
	?>
	
	<h1 class="menuOption3">Entrada</h1>
	...
	<fieldset class="primeira-field">
		...
		<section class="coluna-esquerda">
			<?php
				...
				echo $this->Form->input('Fornecedor.nome', array('value'=>h($xmlArray['nfeProc']['NFe']['infNFe']['emit']['xNome']),'disabled'=>'disabled','type'=>'text','label'=>'Fornecedor:'));
			?>
		</section>
...
<?php
	...
	$dest = $xmlArray['nfeProc']['NFe']['infNFe'];
	...
	echo "</pre>";
		...
		//	echo $items['prod']['cProd'];
			//	foreach($med['prod']['med'] as $meds){
				//	print_r( $meds);
				//	}
			} 
	 	
	 
	 foreach($xmlArray['nfeProc']['NFe']['infNFe']['total']['ICMSTot'] as $impostos){
			//echo $impostos['vST'];
		 }
