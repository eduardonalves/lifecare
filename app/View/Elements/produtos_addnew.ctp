			...
			$('#ProdutoAddForm').submit(function(event){
				...
				$.ajax({
					...
					success: function(data) {
						...
						$(".selectProduto").prepend("<option value='"+data.Produto.id+"' id='"+data.Produto.codigo+"' class='"+data.Produto.unidade+"' rel='"+data.Produto.descricao+"' selected='selected'  >"+data.Produto.nome+"</option>");
						$(".selectProduto  option[value='add-produtos']").remove();
						$('.selectProduto').prepend('<option value="add-produtos">Cadastrar</option>');	
						$('.input .ui-widget-content').val(data.Produto.nome);
...
<header> <!---header--->
	...
	<section> <!---section superior--->
			...
			<section class="coluna-central" >
				<div>
					<?php
						echo $this->Form->input('Produto.nome',array('required'=>'true','class'=>'tamanho-medio', 'label' => 'Nome:', 'maxlength' => '255'));
			...
			<section class="coluna-direita" >
				<div>
					<?php
						...
						echo $this->Form->input('Produto.unidade', array('required'=>'true','type'=>'select','label'=>'Unidade:','options'=>$tiposUnidades));
