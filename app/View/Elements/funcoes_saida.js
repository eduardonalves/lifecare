...
/***************Valida Data*******************************************/
	...
	$("#SaidaData").change(function(){
	    ...
	    if(dfuturoSaida > dataFormat){
		    //saida
		    $('<span id="spanDataFuturoSaida" class="MsgData">Data Emissão Não Pode ser um Dia Futuro</span>').insertAfter('#SaidaData');
...
var princ_cont = 0;
	...
	if(!ValidaCamposBtAdicionar()){
	    ...
	    vlTotalProdAux= $("#SaidaValorTotalProdutos").val().replace(",", ".");
	    ...
	    totNota= $("#SaidaValorTotal").val().replace(",", ".");
		...
		if(auxtipodoc=="Nota"){
			...
			$('#tabela-principal').append('<tr class="valbtconfimar" ><td>'+codigo+'</td><td>'+nome+'</td><td>'+unidade+'</td><td class="descricao"><span title="'+descricao+'">'+descricao+'&nbsp;</span></td><td>'+qtde+'</td><td>'+valor_unitario+'</td><td class=total_clonado'+princ_cont+'>'+valor_totalAux+'</td><td class="table-none">'+cfop+'</td><td class="table-none icms_clonado'+princ_cont+' ">'+valor_icms+'</td><td class="table-none ipi_clonado'+princ_cont+'">'+valor_ipi+'</td> <td><img rel="tooltip" title="'+lotes+'" src="img/icon-dash2.png"/></td> <td><a href="#" id=clonado'+princ_cont+' class="btnRemove">remover</a></td></tr>');
		}else{
			$('#tabela-principal').append('<tr class="valbtconfimar" ><td>'+codigo+'</td><td>'+nome+'</td><td>'+unidade+'</td><td class="descricao"><span title="'+descricao+'">'+descricao+'&nbsp;</span></td><td>'+qtde+'</td><td>'+valor_unitario+'</td><td class=total_clonado'+princ_cont+'>'+valor_totalAux+'</td> <td><img rel="tooltip" title="'+lotes+'" src="img/icon-dash2.png"/></td> <td><a href="#" id=clonado'+princ_cont+' class="btnRemove">remover</a></td></tr>');
		}
		...
		$('fieldset').append('<div class="input number clonado'+princ_cont+'" style="position:absolute"><input name="data[Produtoiten]['+princ_cont+'][valor_total] step="any"  id="ProdutoitenValorTotal'+princ_cont+'valor_total" value="'+valor_totalAux.replace(",", ".")+'" type="hidden"></div> ');
		...
		$('fieldset').append('<div class="input number clonado'+princ_cont+'" style="position:absolute"><input name="data[Produtoiten]['+princ_cont+'][percentual_icms] step="any"  id="ProdutoitenPercentual_icms'+princ_cont+'percentual_icms" value="'+valor_icms.replace(",", ".")+'" type="hidden"></div> ');
		$('fieldset').append('<div class="input number clonado'+princ_cont+'" style="position:absolute"><input name="data[Produtoiten]['+princ_cont+'][percentual_ipi] step="any"  id="ProdutoitenPercentual_ipi'+princ_cont+'percentual_ipi" value="'+valor_ipi.replace(",", ".")+'" type="hidden"></div> ');
		$('fieldset').append('<div class="input number clonado'+princ_cont+'" style="position:absolute"><input name="data[Produtoiten]['+princ_cont+'][valor_unitario] step="any"  id="Produtoitenvalor_unitario'+princ_cont+'valor_unitario" value="'+valor_unitario.replace(",", ".")+'" type="hidden"></div> ');
...
/*********************************Calculo do valor total da nota ***************************/
    ...
    function calcValorNota(){
		...
		outrosAux= $("#SaidaValorOutros").val().replace(",", ".");
		outros=parseFloat(outrosAux);
		 if(isNaN(outros)){
			outros=0;
		}
		
		suguroAux = $("#SaidaValorSeguro").val().replace(",", ".");
		seguro=parseFloat(suguroAux);
		if(isNaN(seguro)){
			seguro=0;
		}
		
		freteAux = $("#SaidaValorFrete").val().replace(",", ".");
		frete=parseFloat(freteAux);
		if(isNaN(frete)){
			frete=0;
		}
		
		produtosAux = $("#SaidaValorTotalProdutos").val().replace(",", ".");
		produtos=parseFloat(produtosAux);
		if(isNaN(produtos)){
			produtos=0;
		}
		
		ipiAux = $("#SaidaValorIpi").val().replace(",", ".");
		ipi=parseFloat(ipiAux);
		if(isNaN(ipi)){
			ipi=0;
		}
		
		icmsAux = $("#SaidaValorIcms").val().replace(",", ".");
	...
	vlIcmsAntigoAux= $("#SaidaValorIcms").val().replace(",", ".");
	...
	vlIpiAntigoAux= $("#SaidaValorIpi").val().replace(",", ".");
...
$('#SaidaValorOutros, #SaidaValorSeguro, #SaidaValorFrete').focusout(function(){
	...
	inttotalNotaAux=0;
	outrosValores=0;
  outros = $('#SaidaValorOutros').val().replace(",", ".");
  seguro = $('#SaidaValorSeguro').val().replace(",", ".");
  frete = $('#SaidaValorFrete').val().replace(",", ".");
  totalNotaAux = $("#SaidaValorTotal").val().replace(",", ".");
...
/************************** Validações Campos de Saidas ********************************/
		...
		$('.autocompleteCliente').focusin(function(){
		    $('.autocompleteCliente input').attr('required','required');
		});
		
		$('.autocompleteCliente').focusout(function(){
		    $('.autocompleteCliente input').removeAttr('required','required');
		});
		
		$('#SaidaNotaFiscal').focusin(function(){
		    $('#SaidaNotaFiscal').attr('required','required');
		});
		
		$('#SaidaData').focusin(function(){
...
/******************** Excluir tabela principal *******************/
	$("body").on("click",'.btnRemove', function(e){
		...
		acumuladorTotalAux = $("#SaidaValorTotalProdutos").val().replace(",", ".");
		acumuladorTotal=parseFloat(acumuladorTotalAux);
		...
		acumuladorIpiAux = $("#SaidaValorIpi").val().replace(",", ".");
		...
		acumuladorIcmsAux = $("#SaidaValorIcms").val().replace(",", ".");
...
/*************************** Função de add Lote *******************************/
	...
	$('.bt-add').bind('click',function(){
		...
		if($('#ProdutoNome').val() ==''){
			...
			$("#LoteEstoque").val("");	
			$("#carregaSelect").load('http://qs.lifecare.vento-consulting.com/lotes/carregalote?numero='+produto_id+'', function(){
