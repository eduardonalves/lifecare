<?php 
	$this->start('css');
	    ...
	    echo $this->Html->css('compras_cotacaovendas.css');
	...
	<header>Dados da Cotação</header>	
		<section class="coluna-esquerda">
			...
			<?php
				...
				echo $this->Form->input('Comoperacao.data_inici',array('label'=>'Data de Início:','class'=>'tamanho-pequeno inputData borderZero','type'=>'text', 'value'=>h(formatDateToView($cotacaovenda['Cotacaovenda']['data_inici'])),'disabled'=>'disabled'));
				echo $this->Form->input('Comoperacao.forma_pagamento',array('type'=>'text','label'=>'Forma de Pagamento:','class'=>'tamanho-pequeno desabilita borderZero', 'value'=>h($cotacaovenda['Cotacaovenda']['forma_pagamento']),'disabled'=>'disabled'));
			?>
		...
		<section class="coluna-central">
			...
			<?php
				if($cotacaovenda['Cotacaovenda']['tipo'] == "Cotacaovenda"){
					$tipoOperacao = "Cotação";
				}else{
					$tipoOperacao = "Pedido";
				}
				
				echo $this->Form->input('Comoperacao.data_fim',array('label'=>'Data de Fim:','class'=>'tamanho-pequeno inputData borderZero','type'=>'text','value'=>h(formatDateToView($cotacaovenda['Cotacaovenda']['data_fim'])),'disabled'=>'disabled')); 
				echo $this->Form->input('Comoperacao.prazo_pagamento',array('label'=>'Prazo de Pagamento:','class'=>'tamanho-pequeno borderZero','type'=>'text','value'=>$cotacaovenda['Cotacaovenda']['prazo_pagamento'],'disabled'=>'disabled'));
		...
		<section class="coluna-direita">
			...
			<?php
				echo $this->Form->input('Comoperacao.id',array('label'=>'Código:','class'=>'tamanho-pequeno borderZero','type'=>'text','value'=>$cotacaovenda['Cotacaovenda']['id'],'disabled'=>'disabled')); 
				echo $this->Form->input('Comoperacao.status',array('label'=>'Status:','type'=>'text','class'=>'tamanho-pequeno borderZero','value'=>$cotacaovenda['Cotacaovenda']['status'],'disabled'=>'disabled'));	
...
<footer>
	...
	<?php
		if($cotacaovenda['Cotacaovenda']['status'] != 'CANCELADO'){
			echo $this->Form->postLink($this->Html->image('botao-excluir2.png',array('id'=>'bt-cancelar','class'=>'bt-esquerda','alt' =>__('Cancelar Cotação'),'title' => __('Cancelar Cotação'))), array('controller' => 'Cotacaovendas','action' => 'cancelarCotacao',$cotacaovenda['Cotacaovenda']['id']),array('escape' => false, 'confirm' => __('Tem certeza que deseja cancelar esta Cotação?', $cotacaovenda['Cotacaovenda']['id'])));
		}
