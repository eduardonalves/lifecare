<div class="pis index">
	<h2><?php echo __('Pis'); ?></h2>
	<table cellpadding="0" cellspacing="0">
	<tr>
			<th><?php echo $this->Paginator->sort('id'); ?></th>
			<th><?php echo $this->Paginator->sort('situacaotribpi_id'); ?></th>
			<th><?php echo $this->Paginator->sort('alq_pis'); ?></th>
			<th><?php echo $this->Paginator->sort('tipodecalculo'); ?></th>
			<th><?php echo $this->Paginator->sort('produto_id'); ?></th>
			<th class="actions"><?php echo __('Actions'); ?></th>
	</tr>
	<?php foreach ($pis as $pi): ?>
	<tr>
		<td><?php echo h($pi['Pi']['id']); ?>&nbsp;</td>
		<td>
			<?php echo $this->Html->link($pi['Situacaotribpi']['id'], array('controller' => 'situacaotribpis', 'action' => 'view', $pi['Situacaotribpi']['id'])); ?>
		</td>
		<td><?php echo h($pi['Pi']['alq_pis']); ?>&nbsp;</td>
		<td><?php echo h($pi['Pi']['tipodecalculo']); ?>&nbsp;</td>
		<td>
			<?php echo $this->Html->link($pi['Produto']['id'], array('controller' => 'produtos', 'action' => 'view', $pi['Produto']['id'])); ?>
		</td>
		<td class="actions">
			<?php echo $this->Html->link(__('View'), array('action' => 'view', $pi['Pi']['id'])); ?>
			<?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $pi['Pi']['id'])); ?>
			<?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $pi['Pi']['id']), null, __('Are you sure you want to delete # %s?', $pi['Pi']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
	</table>
	<p>
	<?php
	echo $this->Paginator->counter(array(
	'format' => __('Page {:page} of {:pages}, showing {:current} records out of {:count} total, starting on record {:start}, ending on {:end}')
	));
	?>	</p>
	<div class="paging">
	<?php
		echo $this->Paginator->prev('< ' . __('previous'), array(), null, array('class' => 'prev disabled'));
		echo $this->Paginator->numbers(array('separator' => ''));
		echo $this->Paginator->next(__('next') . ' >', array(), null, array('class' => 'next disabled'));
	?>
	</div>
</div>
<div class="actions">
	<h3><?php echo __('Actions'); ?></h3>
	<ul>
		<li><?php echo $this->Html->link(__('New Pi'), array('action' => 'add')); ?></li>
		<li><?php echo $this->Html->link(__('List Situacaotribpis'), array('controller' => 'situacaotribpis', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Situacaotribpi'), array('controller' => 'situacaotribpis', 'action' => 'add')); ?> </li>
		<li><?php echo $this->Html->link(__('List Produtos'), array('controller' => 'produtos', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Produto'), array('controller' => 'produtos', 'action' => 'add')); ?> </li>
	</ul>
</div>
