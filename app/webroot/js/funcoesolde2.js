		...
		$.ajax({
			...
			success: function(data) {
				...
				if(numeroLoteAdd !=undefined){
					$('.tabela-lote').append('<tr class="apargarLotes"><td class="val-numero-lote">'+numeroLoteAdd+'</td><td><input class="tamanho-qtde soma" readonly="readonly" value="'+quantidadeLoteAdd+'"/></td><td>'+validadeLoteAdd+'</td><td><a href="" data-qtde="'+quantidadeLoteAdd+'" id=clonado'+lote_cont+' class="btnExcluir">remover</a></td></tr> ');
...
/******************** Valor Unitário, Valor Total(Dados do Produto) *******************/
	...
	function calcularValorTotal(){
		...
		$('#ProdutoitenValorTotal').val(valorTotalProduto);
...
*/
	$(".bt-adicionar").bind('click', function(e){
	var tClas = $('td').hasClass('val-numero-lote');
		...
		if( $('#EntradaNotaFiscal').val() =='' ){
			...
			$('html, body').animate({scrollTop:0}, 'slow');
		}else if( $('#EntradaValorTotalProdutos').val() =='' ){
			$("input[id='EntradaValorTotalProdutos']").addClass('shadow-vermelho');
			$('#spanEntradaValorTotalProdutos').css('display','block');
			$('html, body').animate({scrollTop:0}, 'slow');
		}else if( $('#EntradaValorTotal').val() =='' ){
			$("input[id='EntradaValorTotal']").addClass('shadow-vermelho');
			$('#spanEntradaValorTotal').css('display','block');
			$('html, body').animate({scrollTop:0}, 'slow');
		...
		*/
				//adicionando campos a tabela
				$('#tabela-principal').append('<tr class="valbtconfimar"><td>'+codigo+'</td><td>'+nome+'</td><td>'+unidade+'</td><td class="descricao"><span title="'+descricao+'">'+descricao+'&nbsp;</span></td><td>'+qtde+'</td><td>'+valor_unitario+'</td><td>'+valor_total+'</td><td class="imposto">'+cfop+'</td><td class="imposto">'+valor_icms+'</td><td class="imposto">'+valor_ipi+'</td> <td><img rel="tooltip" title="'+lotes+'" src="/cakephp/img/icon-dash2.png"/></td> <td><a href="#" id=clonado'+princ_cont+' class="btnRemove">remover</a></td></tr>');
...
/************************** Validações Campos de Entradas ********************************/
		...
		$("body").on('focusout','.validacao-entrada',function(){

			if( $('#EntradaNotaFiscal').val() !='' ){
				$( "input[id='EntradaNotaFiscal']" ).removeClass('shadow-vermelho');
				$('#spanEntradaNotaFiscal').css('display','none');
			}

			if( $('#EntradaData').val() !='' ){
				$("input[id='EntradaData']").removeClass('shadow-vermelho');
				$('#spanEntradaData').css('display','none');
			}

			if( $('#EntradaCpfCnpj').val() !='' ){
				$("input[id='EntradaCpfCnpj']").removeClass('shadow-vermelho');
				$('#spanEntradaCpfCnpj').css('display','none');
			}

			if( $('#EntradaValorTotalProdutos').val() !='' ){
				$("input[id='EntradaValorTotalProdutos']").removeClass('shadow-vermelho');
				$('#spanEntradaValorTotalProdutos').css('display','none');
			}

			if( $('#EntradaValorTotal').val() !='' ){
				$("input[id='EntradaValorTotal']").removeClass('shadow-vermelho');
				$('#spanEntradaValorTotal').css('display','none');
			}
		...
		$("body").on('click','#spanVaBtConf',function(){
				$('#spanVaBtConf').css('display','none');
		});

		$("body").on('change','.validacao-entrada',function(){
...
/********************* Barra de Progresso *****************************/
  ...
  /*** Avançar Tela resultado ***/
	$('.bt-confirmar').bind('click',function(){
...
/*
		...
		if(!temclasvalbtconf){
			$('#spanVaBtConf').css('display','block');
		}else{
			id= $(this).attr('id');
