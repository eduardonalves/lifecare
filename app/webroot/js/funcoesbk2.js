...
/********************* Campos Dinâmicos Entrada Manual **************************/
	...
	$("#EntradaData").change(function(){
	    ...
	    if(tipoOperacao == 0){
	    var dfuturo = $("#EntradaData").val();
	    var dataAtual = new Date();
	    var dataFormat = ( (dataAtual.getDate()) + '/' + dataAtual.getMonth()+1) + '/' + dataAtual.getFullYear();

		if(dfuturo > dataFormat){
		    $('<span id="spanDataFuturo" class="MsgData">Data Emissão Não Pode ser um Dia Futuro</span>').insertAfter('#EntradaData');
		    $("#EntradaData").val("");
		}else{
		...
		if(numeroLote == ''){
			...
			success: function(data) {
				...
				if(numeroLoteAdd !=undefined){
					...
					$('fieldset').append('<div class="input number clonado'+lote_cont+'" style="position:absolute"><input name="data[Loteiten]['+lote_cont+'][tipo] step="any"  id="LoteitenTipo'+lote_cont+'tipo" value="ENTRADA" type="hidden"></div> ');
...
/******************** Valor Unitário, Valor Total(Dados do Produto) *******************/
	...
	function calcularValorTotal(){
		...
		$('#ProdutoitenValorTotal').val(valorTotalProdutoAux);
...
var princ_cont = 0;
   ...
   /* $(".bt-adicionar").bind('click', function(e){
	...
	/*		$('#tabela-principal').append('<tr><td>'+codigo+'</td><td>'+nome+'</td><td>'+unidade+'</td><td class="descricao"><span title="'+descricao+'">'+descricao+'&nbsp;</span></td><td>'+qtde+'</td><td>'+valor_unitario+'</td><td>'+valor_total+'</td><td class="imposto">'+cfop+'</td><td class="imposto">'+valor_icms+'</td><td class="imposto">'+valor_ipi+'</td> <td><img rel="tooltip" title="'+lotes+'" src="/cakephp/img/icon-dash2.png"/></td> <td><a href="#" id=clonado'+princ_cont+' class="btnRemove">remover</a></td></tr>');
			...
			$('fieldset').append('<div class="input number clonado'+princ_cont+'" style="position:absolute"><input name="data[Produtoiten]['+princ_cont+'][tipo] step="any"  id="ProdutoitenTipo'+princ_cont+'tipo" value="ENTRADA" type="hidden"></div> ');
	...
	$(".bt-adicionar").bind('click', function(e){
	  ...
	  //  alert(ValidaCamposBtAdicionar());
		...
		return false;
			...
			vlTotalProdAux= $("#EntradaValorTotalProdutos").val();
			...
			$("#EntradaValorTotalProdutos").val(resultTotal);
			
			
			totNota= $("#EntradaValorTotal").val();
		...
		*/
				//adicionando campos a tabela
				$('#tabela-principal').append('<tr class="valbtconfimar" ><td>'+codigo+'</td><td>'+nome+'</td><td>'+unidade+'</td><td class="descricao"><span title="'+descricao+'">'+descricao+'&nbsp;</span></td><td>'+qtde+'</td><td>'+valor_unitario+'</td><td class=total_clonado'+princ_cont+'>'+valor_total+'</td><td class="imposto">'+cfop+'</td><td class="imposto icms_clonado'+princ_cont+' ">'+valor_icms+'</td><td class="imposto ipi_clonado'+princ_cont+'">'+valor_ipi+'</td> <td><img rel="tooltip" title="'+lotes+'" src="img/icon-dash2.png"/></td> <td><a href="#" id=clonado'+princ_cont+' class="btnRemove">remover</a></td></tr>');
...
function calcValorNota(){
		outrosAux= $("#EntradaValorOutros").val();
		outros=parseFloat(outrosAux);
		 if(isNaN(outros)){
			outros=0;
		}
		
		suguroAux = $("#EntradaValorSeguro").val();
		seguro=parseFloat(suguroAux);
		if(isNaN(seguro)){
			seguro=0;
		}
		
		freteAux = $("#EntradaValorFrete").val();
		frete=parseFloat(freteAux);
		if(isNaN(frete)){
			frete=0;
		}
		
		produtosAux = $("#EntradaValorTotalProdutos").val();
		produtos=parseFloat(produtosAux);
		if(isNaN(produtos)){
			produtos=0;
		}
		
		ipiAux = $("#EntradaValorIpi").val();
		ipi=parseFloat(ipiAux);
		if(isNaN(ipi)){
			ipi=0;
		}
		
		icmsAux = $("#EntradaValorIcms").val();
		...
		$('#EntradaValorTotal').val(novoTotalNota);
...
function somarIcmsIpi(){
	...
	vlIcmsAntigoAux= $("#EntradaValorIcms").val();
	...
	$("#EntradaValorIcms").val(vlIcmsNovo);
	
	

	vlIpiAntigoAux= $("#EntradaValorIpi").val();
...
$('#EntradaValorOutros, #EntradaValorSeguro, #EntradaValorFrete').focusout(function(){
	...
	inttotalNotaAux=0;
	outrosValores=0;
  outros = $('#EntradaValorOutros').val();
  seguro = $('#EntradaValorSeguro').val();
  frete = $('#EntradaValorFrete').val();
  totalNotaAux = $("#EntradaValorTotal").val();
...
/************************** Validações Campos de Entradas ********************************/
		...
		$('#EntradaNotaFiscal').on('focusin',function(){
		     $('#EntradaNotaFiscal').attr('required','required');
		});

		$('#EntradaData').on('focusin',function(){
...
/**************** apagar campos qd muda de vale/nota ****************/
    function apagarCamposEntrada(){

	//apaga msg de validação
	$( "input[id='EntradaNotaFiscal']" ).removeClass('shadow-vermelho');
	$('span[id="spanEntradaNotaFiscal"]').remove();
	$("input[id='EntradaData']").removeClass('shadow-vermelho');
	$('span[id="spanEntradaData"]').remove();
	...
	$("#LoteQuantidade").val("");
	$("#LoteId").val("");
	$('#EntradaChaveAcesso').val('');
	$('#EntradaNotaFiscal').val('');
	$('#EntradaData').val('');
	$('#EntradaValorIpi').val('');
	$("#EntradaValorTotalProdutos").val("");
	$("#EntradaValorOutros").val("");
	$("#EntradaValorTotal").val("");
	$("#EntradaValorSeguro").val("");
	$("#EntradaValorIcms").val("");
	$("#EntradaValorFrete").val(""); */
    }
...
/******************** Excluir tabela principal *******************/
	$("body").on("click",'.btnRemove', function(e){
		...
		acumuladorTotalAux = $("#EntradaValorTotalProdutos").val();
		acumuladorTotal=parseFloat(acumuladorTotalAux);
		...
		$("#EntradaValorTotalProdutos").val(novoVal);
		
		
		
		
		
		acumuladorIpiAux = $("#EntradaValorIpi").val();
		...
		$("#EntradaValorIpi").val(novoValIpi);
		
		acumuladorIcmsAux = $("#EntradaValorIcms").val();
		...
		$("#EntradaValorIcms").val(novoValIcms);		
...
/***************************** Função Validar Campos Botão Confirmar *****************************/
	function ValidaCamposBtConfirmar(){
		...
		if( $('#EntradaNotaFiscal').val() =='' ){
		    ...
		    return false;
		}else if( $('#EntradaData').val() =='' ){
		    $('span[id="spanEntradaData"]').remove();
		    $('span[id="spanDataFuturo"]').remove();
		    $('<span id="spanEntradaData" class="MsgData">Preencha o campo Data Emissão</span>').insertAfter('input[id="EntradaData"]');
		    $("input[id='EntradaData']").addClass('shadow-vermelho');
		    $('html, body').animate({scrollTop:0}, 'slow');
			return false;
		}else if( $('#EntradaCpfCnpj').val() =='' ){
...
/*
		if( $('#EntradaNotaFiscal').val() =='' ){
			$("input[id='EntradaNotaFiscal']").addClass('shadow-vermelho');
			$('#spanEntradaNotaFiscal').css('display','block');
			$('html, body').animate({scrollTop:0}, 'slow');
		}else if( $('#EntradaData').val() =='' ){
			$("input[id='EntradaData']").addClass('shadow-vermelho');
			$('#spanEntradaData').css('display','block');
			$('html, body').animate({scrollTop:0}, 'slow');
		}else if( $('#EntradaCpfCnpj').val() =='' ){
			$("input[id='EntradaCpfCnpj']").addClass('shadow-vermelho');
			$('#spanEntradaCpfCnpj').css('display','block');
			$('html, body').animate({scrollTop:0}, 'slow');
...
/*************Funcões para Entrada Manual*******************/
 $('.bt-preencher').bind('click', function(){
	...
	produtoCfop = $(".selectProduto option:selected").attr('title');
