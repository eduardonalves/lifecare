 ...
 */
	public function add() {
		...
		if ($this->request->is('post')) {
			...
			if($this->request->data['Contaspagar']['repetir'] == 1){
				
				for ($i =0; $i >= $this->request->data['Contaspagar']['repeticoes'] ; $i++){
					
					$this->Contaspagar->create();
					
					
					$this->lifecareDataFuncs->formatDateToBD($this->request->data['Contaspagar']['data_emissao']);
					$periodo = explode('-', $this->request->data['Contaspagar']['data_emissao']);
					$ano =$periodo[0];
					$mes = $periodo[1];
					$dia = $periodo[2];
					$novaDataEmissao = mktime(0, 0, 0, ($mes + $i), $dia, $ano);
				 	$this->request->data['Contaspagar']['data_emissao'] = $novaDataEmissao;
					...
					if ($this->Contaspagar->saveAll($this->request->data)) {
						...
						foreach($parcelasEnviadas as $parcelasEnviada){
							...
							$periodo = explode('-', $parcelasEnviada['data_vencimento']);
							$ano =$periodo[0];
							$mes = $periodo[1];
							$dia = $periodo[2];
							$novaDataVencimento = mktime(0, 0, 0, ($mes + $i), $dia, $ano);
						 	$parcelasEnviada['data_vencimento'] = $novaDataVencimento;
							
							$this->lifecareDataFuncs->formatDateToBD($parcelasEnviada['data_pagamento']);
							$periodo = explode('-', $parcelasEnviada['data_pagamento']);
							$ano =$periodo[0];
							$mes = $periodo[1];
							$dia = $periodo[2];
							$novaDataPagamento = mktime(0, 0, 0, ($mes + $i), $dia, $ano);
						 	$parcelasEnviada['data_pagamento'] = $novaDataPagamento;
